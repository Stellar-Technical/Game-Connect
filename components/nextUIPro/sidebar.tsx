...
import { Listbox, Tooltip, ListboxItem, ListboxSection, Image } from "@nextui-org/react"
...
const Sidebar = React.forwardRef<HTMLElement, SidebarProps>(({ items, isCompact, defaultSelectedKey, onSelect, hideEndContent, sectionClasses: sectionClassesProp = {}, itemClasses: itemClassesProp = {}, iconClassName, classNames, className, ...props }, ref) => {
    ...
    console.log("selected :", [selected], defaultSelectedKey)
        ...
        (item: SidebarItem) => {
            ...
            return (
                <ListboxItem
                    ...
                    startContent={isCompact || isNestType ? null : item.icon ? <Icon className={cn("text-default-500 group-data-[selected=true]:text-foreground", iconClassName)} icon={item.icon} width={24} /> : item.startContent ?? null}
                ...
                >
                    ...
                    {!isCompact && isNestType ? (
                        <Accordion className={"p-0"} key={item.key}>
                            <AccordionItem
                                key={item.key}
                                aria-label={item.title}
                                data-open={defaultSelectedKey.split(":")[1] === item.key ? "true" : "false"}
                                classNames={{
                                    heading: "pr-3",
                                    trigger: "p-0",
                                    content: "py-0 pl-4",
                                ...
                                title={
                                    item.icon && !item.iconImage ? (
                                        ...
                                        <div className={"flex h-11 items-center gap-2 px-2 py-1.5"}>
                                            <Image width={28} height={28} src={item.iconImage} alt="NextUI hero Image" />
                                            <span className="text-small font-medium text-default-500 group-data-[selected=true]:text-foreground">{item.title}</span>
                                        </div>
                                    ) : (
                            ...
                            >
                                {item.items && item.items?.length > 0 ? (
                                    <Listbox
                                        key={item.key}
                                        className={`mt-0.5 ${defaultSelectedKey != (item.key as string) ? "text-default-500" : ""}`}
                                        classNames={{
                                            list: cn("border-l border-default-200 pl-4"),
                                        }}
                                        items={item.items}
                                        variant="flat"
                                        aria-label="sublistMenu"
                                    >
    ...
    const renderItem = React.useCallback(
        (item: SidebarItem) => {
            ...
            return (
                <ListboxItem {...item} key={item.key} endContent={isCompact || hideEndContent ? null : item.endContent ?? null} startContent={isCompact ? null : item.icon ? <Icon className={cn("text-default-500 group-data-[selected=true]:text-foreground", iconClassName)} icon={item.icon} width={24} /> : item.startContent ?? null} textValue={item.title} title={isCompact ? null : item.title}>
                    {isCompact ? (
                        <Tooltip content={item.title} placement="right">
                            <div className="flex w-full items-center justify-center">{item.icon ? <Icon className={cn("text-default-500 group-data-[selected=true]:text-foreground", iconClassName)} icon={item.icon} width={24} /> : item.startContent ?? null}</div>
                        </Tooltip>
